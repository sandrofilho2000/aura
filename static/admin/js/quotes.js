...
function setQuoteTotalPrice(){
    let discount = 0
    if(document.querySelector("#quote_form input#id_discount")){
        discount = document.querySelector("#quote_form input#id_discount").value
    }else{
        discount = passToNumber(document.querySelector("#quote_form .field-discount .readonly").innerText)
    }
    ...
    document.querySelector("#quote_form .field-total_price .readonly").innerText = passToCurrency(total)
    document.querySelector("#quote_form .field-subtotal_price .readonly").innerText = passToCurrency(subtotal)
    ...
    if(document.querySelectorAll(".dynamic-products .field-supplier_price p").length){
        ...
        document.querySelector("#quote_form .field-supplier_price .readonly").innerHTML = `${passToCurrency(supplier_price)} <small>(${supplier_price_percentage.toFixed(2)}%)</small>`
        document.querySelector("#quote_form .field-profit .readonly").innerHTML = `${passToCurrency(profit)} <small>(${profit_percentage.toFixed(2)}%)</small>`
    }
...
function addEventListenerToRemoveItemsBtn(){
    document.querySelectorAll("table tr td.delete a.inline-deletelink").forEach((item)=>{
        item.addEventListener("click", setQuoteTotalPrice)
    })
}
...
document.addEventListener("DOMContentLoaded", function(){
    setTimeout(()=>{
        if(document.querySelectorAll(".field-discount").length){
            ...
            document.querySelector("#quote_form input#id_discount")?.addEventListener("change", setQuoteTotalPrice)
